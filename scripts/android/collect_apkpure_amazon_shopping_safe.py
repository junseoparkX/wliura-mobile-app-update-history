...
APP_NAME_FULL = "Amazon Shopping"
PLATFORM = "Android"
DEVELOPER_COMPANY = "Amazon Mobile LLC"
APP_CATEGORY = "Shopping"
INITIAL_APP_RELEASE_DATE = "December 10, 2014"

VERSIONS_URL = "https://apkpure.com/amazon-shopping/com.amazon.mShop.android.shopping/versions"
BASE_URL = "https://apkpure.com"
FALLBACK_DETAIL_BASE_URL = "https://apkpure.com/amazon-shopping/com.amazon.mShop.android.shopping/download/"

# If this script is in:
# project/code/collect_apkpure_amazon_shopping_safe.py
# then data will be saved to:
# project/data/raw/android/...
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_DIR = SCRIPT_DIR.parent.parent
DEFAULT_OUTPUT = PROJECT_DIR / "data" / "raw" / "android" / "amazon_shopping_apkpure_versions_enriched.csv"
...
# Supports both forms:
# Amazon Shopping 32.9.0.100 / 174.4 MB / Apr 29, 2026 / Download
# Amazon Shopping 32.8.2.100 / XAPK / 170.9 MB / Apr 21, 2026 / Download
...
WHATS_NEW_RE = re.compile(
    ...
    r"(?P<desc>.*?)"
    r"(?=Download APK|Download XAPK|0\s*/\s*\d+|Scan Result|Old Versions|Amazon Shopping Screenshots|All Versions|Featured Reviews|Related Tags|Also available|More Apps|Trending Apps|Amazon Shopping Articles|APKPure)",
    re.IGNORECASE | re.DOTALL,
)
...
def remove_apkpure_boilerplate(desc: str, version: str) -> str:
    ...
    boilerplate_patterns = [
        rf"{APP_NAME} is a retail app that lets you discover, compare, and buy products.*?phone\.?,?",
        rf"Download and install old versions of {APP_NAME}.*?features!?",
        rf"{APP_NAME} old version {re.escape(version)} was released on .*? by Amazon Mobile LLC\.?,?",
        r"The app download size is .*?\.?,?",
        rf"We recommend you download the latest version of {APP_NAME}.*?performance\.?,?",
        r"Check out the detailed comparison between the latest and old version of Amazon Shopping.*",
    ]
...
def infer_update_categories(description: str, raw_description: str):
    ...
    if any(x in text for x in ["payment", "checkout", "subscription", "premium", "monetization", "purchase", "ads", "cart", "shopping cart", "order", "orders", "daily deals"]):
        categories.append("Payments / monetization")
    if any(x in text for x in ["personalization", "recommendations", "recommended", "discover", "discovery", "search", "shortcuts", "price", "price drops"]):
    ...
    if any(x in text for x in ["new feature", "introducing", "now you can", "added", "launch", "shortcut", "shortcuts", "view in your room", "barcode", "visual search"]):
