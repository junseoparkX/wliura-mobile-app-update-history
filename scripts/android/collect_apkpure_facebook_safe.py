...
# The Facebook app for Android was initially released in September 2009.
INITIAL_APP_RELEASE_DATE = "September 2009"

PACKAGE_NAME_FALLBACK = "com.facebook.katana"

VERSIONS_URL = "https://apkpure.com/facebook-app/com.facebook.katana/versions"
BASE_URL = "https://apkpure.com"
FALLBACK_DETAIL_BASE_URL = "https://apkpure.com/facebook-app/com.facebook.katana/download/"

# If this script is in:
# project/code/collect_apkpure_facebook_safe.py
# then data will be saved to:
# project/data/raw/android/...
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_DIR = SCRIPT_DIR.parent.parent
DEFAULT_OUTPUT = PROJECT_DIR / "data" / "raw" / "android" / "facebook_apkpure_versions_enriched.csv"
...
# Facebook APKPure rows can look like:
# Facebook 559.1.0.52.72 / 95.5 MB / May 5, 2026 / Download
# Facebook 558.0.0.70.72 / APK / 146.0 MB / Apr 29, 2026 / Download
...
WHATS_NEW_RE = re.compile(
    ...
    r"(?P<desc>.*?)"
    r"(?=Download\s+(?:APK|XAPK|APKS)|\d+\s*/\s*\d+|Scan Result|Old Versions of Facebook|Facebook Screenshots|All Versions|Featured Reviews|Related Tags|APKPure)",
    re.IGNORECASE | re.DOTALL,
)
...
def parse_prefixed_line(lines, prefix):
    """Handles lines such as Package Name:com.facebook.katana or SHA-256:<hash>."""
...
def find_value_after_label_allow_blank(lines, label):
    """Finds the next non-empty line after a label. Useful for 'Verified by:' / APKPURE."""
    label_lower = label.lower()
    for i, line in enumerate(lines):
        if line.lower().rstrip(":") == label_lower.rstrip(":"):
            for j in range(i + 1, min(i + 4, len(lines))):
                candidate = lines[j].strip()
                if candidate and not candidate.lower().startswith("show more"):
                    return candidate
    return ""
...
def parse_top_detail_metadata(lines, full_text):
    ...
    if detail_security_ratio and not re.fullmatch(r"\d+\s*/\s*\d+", detail_security_ratio):
        detail_security_ratio = ""
    ...
    # Security / Teen / Android 6.0+ / Android OS
    top_content_rating = ""
    for i, line in enumerate(lines):
...
def parse_more_app_info(lines):
    """
    Parses Facebook Additional APP Information fields:
    Latest Version
    428.0.0.47.67
    Uploaded by
    Google LLC or APKPure uploader account
    Requires Android
    Android 9.0+
    Category
    Free Social App
    ...
    APKPure may not show Package Name or App Permissions on every Facebook detail page,
...
def parse_security_info(lines):
    scan_result = ""
    scan_date = ""
    security_check_completed = "Yes" if any(
        line.lower() == "security check completed" for line in lines
    ) else ""
    ...
    # Some Facebook APKPure detail pages show 'Security Check Completed' plus
    # No Virus / No Spyware / No Malware, but no explicit 'Scan Result:' line.
    if not scan_result and security_check_completed:
        no_flags = all(
            label in {line.strip() for line in lines}
            for label in ["No Virus", "No Spyware", "No Malware"]
        )
        scan_result = "Secure" if no_flags else "Security Check Completed"
    ...
    return {
        ...
        "verified_by": find_value_after_label_allow_blank(lines, "Verified by"),
    }
...
def parse_versions_from_text(text: str):
    ...
    for match in VERSION_CARD_RE.finditer(text):
        ...
        detail_url = FALLBACK_DETAIL_BASE_URL + version
        detail_url_version_slug = extract_version_from_url(detail_url)
...
def build_base_row(
    ...
    return {
        ...
        # APKPure security verification fields
...
def remove_apkpure_boilerplate(desc: str, version: str) -> str:
    ...
    boilerplate_patterns = [
        rf"{APP_NAME} is a social networking app that helps you connect and discover content from people and communities\.?",
        rf"Download and install old versions of {APP_NAME} that suits your device model and enjoy your favorite features!?",
        rf"{APP_NAME} old version {re.escape(version)} was released on .*? by Meta Platforms, Inc\. ?",
        ...
        r"Check out the detailed comparison between the latest and old version of Facebook.*",
    ]
...
def collect_from_web(max_versions=100, max_detail=100, sleep_seconds=1.0):
    ...
    print(f"Found {total_found} versions")

    if max_versions is not None:
        rows = rows[:max_versions]
        if total_found < max_versions:
            print(f"Warning: requested up to {max_versions} version rows, but only {total_found} were found.")
        else:
            print(f"Keeping first {len(rows)} version rows.")
...
def collect_from_local_text(versions_text_path, detail_text_dir=None, link_map_csv=None, max_versions=100, max_detail=100):
    ...
    total_found = len(rows)
    print(f"Found {total_found} versions from local text")

    if max_versions is not None:
        rows = rows[:max_versions]
        if total_found < max_versions:
            print(f"Warning: requested up to {max_versions} version rows, but only {total_found} were found in the local text.")
        else:
            print(f"Keeping first {len(rows)} version rows.")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Maximum number of version rows to keep. Default is 100. Use -1 for all.",
    )
    ...
    parser.add_argument(
        "--detail-text-dir",
        default=None,
        help="Optional folder containing browser-copied detail pages named like 559.1.0.52.72.txt.",
    )
