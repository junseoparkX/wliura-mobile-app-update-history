...
APP_NAME = "TikTok"
PLATFORM = "Android"
DEVELOPER_COMPANY = "TikTok Pte. Ltd."
APP_CATEGORY = "Social"

# For strict research use, verify this against your chosen official source.
# TikTok launched internationally in September 2017; Douyin/TikTok platform history often traces to Sept. 20, 2016.
INITIAL_APP_RELEASE_DATE = "September 2017"

PACKAGE_NAME_FALLBACK = "com.zhiliaoapp.musically"

VERSIONS_URL = "https://apkpure.com/tiktok-musically-2025/com.zhiliaoapp.musically/versions"
BASE_URL = "https://apkpure.com"
FALLBACK_DETAIL_BASE_URL = "https://apkpure.com/tiktok-musically-2025/com.zhiliaoapp.musically/download/"

# If this script is in:
# project/code/collect_apkpure_tiktok_safe.py
# then data will be saved to:
# project/data/raw/android/...
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_DIR = SCRIPT_DIR.parent.parent
DEFAULT_OUTPUT = PROJECT_DIR / "data" / "raw" / "android" / "tiktok_apkpure_versions_enriched.csv"
...
# TikTok APKPure rows can look like:
# TikTok 45.0.42 / Latest / APK / 363.8 MB / Apr 28, 2026 / Download
# TikTok 45.0.2 / APK / XAPK / 517.9 MB / Apr 30, 2026 / Download
...
WHATS_NEW_RE = re.compile(
    ...
    r"(?P<desc>.*?)"
    r"(?=Download\s+(?:APK|XAPK|APKS)|\d+\s*/\s*\d+|Scan Result|Old Versions of TikTok|TikTok Screenshots|All Versions|Featured Reviews|Related Tags|APKPure)",
    re.IGNORECASE | re.DOTALL,
)
...
def parse_top_detail_metadata(lines, full_text):
    ...
    # In TikTok detail pages, Content Rating can appear before Android OS:
...
def parse_more_app_info(lines):
    """
    Parses TikTok Additional APP Information fields:
    Latest Version
    45.0.42
    Uploaded by
    TikTok Pte. Ltd.
    ...
    APKPure may not show Package Name or App Permissions on every TikTok detail page,
...
def remove_apkpure_boilerplate(desc: str, version: str) -> str:
    ...
    boilerplate_patterns = [
        rf"{APP_NAME} is a social video app that lets you watch, create, go LIVE, and shop\.?",
        rf"Download and install old versions of {APP_NAME} that suits your device model and enjoy your favorite features!?",
        rf"{APP_NAME} old version {re.escape(version)} was released on .*? by TikTok Pte\. Ltd\. ?",
        ...
        r"Check out the detailed comparison between the latest and old version of TikTok.*",
    ]
...
def infer_update_categories(description: str, raw_description: str):
    ...
    if any(x in text for x in ["payment", "checkout", "subscription", "monetization", "purchase", "ads", "shop", "shopping"]):
...
def main():
    ...
    parser.add_argument(
        "--detail-text-dir",
        default=None,
        help="Optional folder containing browser-copied detail pages named like 45.0.42.txt.",
    )
