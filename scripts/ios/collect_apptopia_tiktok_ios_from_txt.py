...
APP_NAME_SHORT = "TikTok"
PLATFORM = "iOS"
DEVELOPER_COMPANY = "TikTok Ltd."
APP_CATEGORY = "Entertainment; Photo & Video"
INITIAL_APP_RELEASE_DATE = "April 2, 2014"
IOS_APP_ID = "835599320"
SOURCE_URL = "https://apptopia.com/ios/app/835599320/about"

# Expected project structure:
# project/
#   code/collect_apptopia_tiktok_ios_from_txt.py
#   data/ios_txt/tiktok_apptopia_about.txt
#   data/raw/ios/tiktok_apptopia_versions_enriched.csv
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_DIR = SCRIPT_DIR.parent.parent
DEFAULT_SOURCE_TEXT = PROJECT_DIR / "data" / "ios_txt" / "tiktok_apptopia_about.txt"
DEFAULT_OUTPUT = PROJECT_DIR / "data" / "raw" / "ios" / "tiktok_apptopia_versions_enriched.csv"
...
FIELDNAMES = [
    ...
    "has_for_you_feed_feature",
    "has_live_feature",
    "has_shop_feature",
    "has_creation_tools_feature",
    "has_music_sounds_feature",
    "has_effects_stickers_feature",
    "has_social_sharing_feature",
    "has_personalization_feature",
    "has_safety_privacy_feature",
    "has_family_pairing_feature",
...
def parse_description_features(description: str):
    ...
    flags = {
        "has_for_you_feed_feature": any(x in text for x in ["for you feed", "for you", "personalized discovery"]),
        "has_live_feature": any(x in text for x in ["live", "stream", "streaming"]),
        "has_shop_feature": any(x in text for x in ["shop", "shopping", "products", "brands", "small businesses"]),
        "has_creation_tools_feature": any(x in text for x in ["creation tools", "editing tools", "record", "trim", "merge", "create"]),
        "has_music_sounds_feature": any(x in text for x in ["sounds", "music", "audio"]),
        "has_effects_stickers_feature": any(x in text for x in ["effects", "stickers", "filters"]),
        "has_social_sharing_feature": any(x in text for x in ["share", "follow", "community", "creators"]),
        "has_personalization_feature": any(x in text for x in ["personalized", "learns what you enjoy", "based on what you watch", "current interests"]),
        "has_safety_privacy_feature": any(x in text for x in ["safety", "privacy", "private accounts", "content filters", "terms", "policy"]),
        "has_family_pairing_feature": "family pairing" in text,
        "has_subscription_feature": any(x in text for x in ["subscribe", "subscription", "auto-renew"]),
    }
...
def infer_update_categories(description: str):
    ...
    if any(x in text for x in ["bug", "fix", "fixed", "squashed", "crash", "stability", "performance", "optimization", "optimizations", "improvement", "improvements", "polish"]):
        categories.append("Bug fixes / performance improvements")

    if any(x in text for x in ["ui", "design", "layout", "interface", "redesign", "visual", "feed", "browsing experience", "profile page"]):
        categories.append("UI / design changes")

    if any(x in text for x in ["privacy", "data policy", "consent", "tracking", "privacy settings", "private", "content filters"]):
        categories.append("Privacy / data policy changes")

    if any(x in text for x in ["ai", "artificial intelligence", "computer vision", "generative", "ai-powered"]):
        categories.append("AI-related features")

    if any(x in text for x in ["payment", "checkout", "subscription", "monetization", "purchase", "coins", "shop", "shopping", "products", "brands", "deals"]):
        categories.append("Payments / monetization")

    if any(x in text for x in ["personalized", "recommendation", "recommendations", "discover", "discovery", "for you", "feed", "find", "tag your friends", "comments", "creator"]):
        categories.append("Personalization / recommendations")

    if any(x in text for x in ["security", "account safety", "login", "password", "two-factor", "2fa", "safe", "safety", "family pairing", "screen time", "data", "protect your account"]):
    ...
    if any(
        x in text
        for x in [
            "new feature", "introducing", "now you can", "added", "add", "launch", "launched",
            "shoot", "post videos", "post videos and photos", "sound", "sounds", "stickers",
            "effects", "transition effects", "filters", "live", "comments", "share", "sharing",
            "creative tools", "creator tools", "voice effects", "text effect", "timer", "duet",
            "reaction", "background upload", "thumbnails", "video quality", "profile video",
        ]
    ):
...
def classify_feature_specificity(description: str):
    ...
    generic_notes = {
        ...
        "update with better performance",
        "we've rolled out performance improvements and bug fixes. thanks for using tiktok!",
        "we've been working on making your experience on tiktok better with updates and bug fixes.",
        "we've fixed some bugs and updated features to improve your experience on tiktok.",
        "easily post videos and photos on tiktok with bug fixes and performance improvements.",
...
def make_standardized_summary(description: str, categories: str):
    ...
    routine_phrases = [
        ...
        "better performance",
        "squashed bugs",
        "updated features to improve your experience",
        "polish up tiktok",
    ]

    if any(x in lower for x in routine_phrases) and len(desc.split()) <= 24:
        return "Routine maintenance update focused on bug fixes, performance optimization, or general app experience improvements."

    if "shoot" in lower or "post videos" in lower:
        return "Creation workflow update making video shooting or posting easier."

    if "feed" in lower or "browsing" in lower:
        return "Viewing experience update improving the TikTok feed or video browsing experience."

    if "sound" in lower or "music" in lower or "voice effects" in lower:
        return "Creative audio update related to sounds, music, sound search, or voice effects."

    if "stickers" in lower or "effects" in lower or "filters" in lower or "transition" in lower:
        return "Creative tools update adding or improving stickers, effects, filters, or transition tools."

    if "share" in lower or "sharing" in lower:
        return "Social sharing update making it easier to share TikTok content, effects, LIVE streams, or profiles."

    if "comments" in lower or "tag your friends" in lower:
        return "Social interaction update related to comments, tagging friends, or creator engagement."

    if "shop" in lower or "shopping" in lower or "coins" in lower or "subscription" in lower:
        return "Commerce or monetization-related update involving shopping, coins, purchases, or subscriptions."

    if "privacy" in lower or "security" in lower or "account safety" in lower or "protect your account" in lower:
        return "Safety or privacy update involving account protection, privacy controls, or security settings."

    if "artificial intelligence" in lower or "computer vision" in lower:
        return "AI-related product update using artificial intelligence or computer vision for discovery or recommendation features."

    if "kazakh" in lower or "supported" in lower:
        return "Localization update adding or improving language support."
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse browser-copied Apptopia TikTok iOS page text into a CSV under data/raw/ios."
    )
    parser.add_argument(
        "--source-text",
        default=str(DEFAULT_SOURCE_TEXT),
        help="Path to browser-copied Apptopia text. Default: data/ios_txt/tiktok_apptopia_about.txt",
    )
    parser.add_argument(
        "--output",
        default=str(DEFAULT_OUTPUT),
        help="Output CSV path. Default: data/raw/ios/tiktok_apptopia_versions_enriched.csv",
    )
    parser.add_argument(
        "--start-date",
        default="2024-01-01",
    ...
    if not source_path.exists():
        raise FileNotFoundError(
            ...
            "Expected structure: project/data/ios_txt/tiktok_apptopia_about.txt"
        )
